...
class RouterDriver(object):
    ...
    def dispense_ticks(self, dispenser, ticks, speed=255):
        ...
        for i in range(0, len(self.dispenser_ports)):
            if self.dispenser_ports[i] == dispenser:
                port = i;
        print "ports: " + str(self.dispenser_ports)
        print "Dispenser IDs: "+ str(self.dispenser_ids)
        return self._send_packet16(port, PACKET_TICK_SPEED_DISPENSE, ticks, speed)

    def dispense_ml(self, dispenser, ml, speed=255):
        if self.software_only: return True
        ticks = ml*2.78
        return self.dispense_ticks(dispenser, ticks, speed)
    ...
    def _send_packet(self, dest, packet):
        ...
        if ack == PACKET_CRC_FAIL: 
            self.log.info("send packet: packet ack crc fail")
            return False
        if ack == PACKET_ACK_TIMEOUT: 
            self.log.info("send_packet: ack timeout")
            return False
        if ack == PACKET_ACK_INVALID: 
            self.log.info("send_packet: dispenser received invalid packet")
            return False
        if ack == PACKET_ACK_INVALID_HEADER: 
            self.log.info("send_packet: dispenser received invalid header")
            return False
        if ack == PACKET_ACK_HEADER_IN_PACKET:
            self.log.info("send_packet: header in packet error")
            return False

        # if we get an invalid ack code, it might be ok. 
        self.log.info("send_packet: Invalid ACK code %d" % ord(ch))
        return False
